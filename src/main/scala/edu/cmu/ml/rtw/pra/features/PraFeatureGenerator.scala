...
class PraFeatureGenerator(
  ...
  def createTestMatrix(relation: String,dataset: Dataset[NodePairInstance]):FeatureMatrix={
    isTraining=false
    relationName=relation
    createTestMatrix(dataset)
  }

  override def createTrainingMatrix(relation:String,dataset: Dataset[NodePairInstance]):FeatureMatrix={
    isTraining=true
    relationName=relation
